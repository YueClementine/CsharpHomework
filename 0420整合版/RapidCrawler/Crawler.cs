...
namespace RapidCrawler {
  class Crawler {
    public event Action<Crawler> CrawlerStopped;
    public event Action<Crawler, int,string, string> Crawl;
    ...
    public static readonly string urlParse = @"^(?<site>https?://(?<host>[\w\d.]+)(:\d+)?($|/))([\w\d]+/)*(?<file>[^#?]*)";
    ...
    public void Startup() {
      ...
      Crawl +=(crawler,index,url,info)=> { complatedCount++; };
      ...
      while (tasks.Count<MaxPage) {
        ...
        Task task=Task.Run(() => Parse(url, index));
        tasks.Add(task);
      }
    ...
    private void Parse(string url,int index) {
      try {
        string html = DownLoad(url,index);
        urls[url] = true;
        Parse(html, url);
        Crawl(this,index, url, "success");
      }catch (Exception ex) {
        Crawl(this,index, url, "Error:" + ex.Message);
      }
    }
    ...
    static private string FixUrl(string url, string baseUrl) {
      ...
      if (url.StartsWith("/")) {
        Match urlMatch = Regex.Match(baseUrl, urlParse);
